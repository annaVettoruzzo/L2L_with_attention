...
def main(args):
    ...
    print(f"Method: {config_params['method']}, multihead: {config_params['multihead']}")

    # For saving models
    if config_params['multihead']:
        PATH = Path(f"_saved_models_final/{config_params['dataset']}/seed{config_params['seed']}/{config_params['method']}_multihead")
    else:
    ...
    if config_params['method'] == 'icarl':
        train_dataset, test_dataset = dataset.split_task_construction(config_params['dataset'], task_labels, class_il=True)
    else:
        train_dataset, test_dataset = dataset.split_task_construction(config_params['dataset'], task_labels)
    utils.save_object(task_labels, PATH / 'task_labels.json')
    ...
    if config_params['method'] == 'proposed':
        ...
        model = base_models.ConvNet(config_params, n_heads, n_classes=config_params['n']).to(config_params['device'])
        nweights = utils.count_parameters(model)
        pretrained_te_path = f"_saved_models/{config_params['dataset']}/task_encoder_svhn.pth"
        transformer_model = transformer_models.TransformerModel(nweights, config_params, pretrained_te_path).to(config_params['device'])
    ...
    fwt = utils.compute_fwt(future_tasks_acc)  # FIXME: implement it as in methods
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr', type=float, default=1e-3,
                        help='Learning rate')
    parser.add_argument('--lr_transformer', type=float, default=1e-3,
                        help='Learning rate transformer')
    parser.add_argument('--lr_inner', type=float, default=1e-2,
                        help='Learning rate inner loop')
    parser.add_argument('--lr_inner_transformer', type=float, default=1e-2,
                        help='Learning rate inner loop transformer')
    parser.add_argument('--lr_inner_test', type=float, default=1e-3,
    ...
    parser.add_argument('--predict_bn', action="store_true",
                        help='Predict batch normalization parameters')
    parser.add_argument('--unique_te', action="store_false",
    ...
    parser.add_argument('--clamping_transformer', type=float, default=1.0,
                        help='Clamping parameters for transformer output')
    parser.add_argument('--spatial', type=int, default=None,
