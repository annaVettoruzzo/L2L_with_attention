...
def main(args):
    ...
    print(f"Method: {config_params['method']}, multihead: {config_params['multihead']}, seed: {config_params['seed']}")
    ...
    if config_params['multihead']:
        ...
        PATH = Path(f"_saved_models_final/{config_params['dataset']}/seed{config_params['seed']}/{config_params['method']}_3x3_64_1shot")
    ...
    n_tasks = 10
    train_dataset, test_dataset, rotation_angles = dataset.rotated_task_construction(n_tasks)
    utils.save_object(rotation_angles, PATH / 'rotation_angles.json')
    ...
    if config_params['method'] == 'proposed':
        ...
        pretrained_te_path = f"_saved_models/splitmnist/task_encoder_svhn.pth"
...
if __name__ == '__main__':
    ...
    parser.add_argument("--multihead", action="store_true",
                        help="Enable multihead")  # True when specified, otherwise default to False
    ...
    parser.add_argument('--lr_inner_test', type=float, default=1e-4,
    ...
    parser.add_argument('--clamping_transformer', type=float, default=2.0,
